...
function calculateWinrate() {
    ...
    // 根据组排情况调整监管者的elo分数
    let adjustedHunterX = hunterX;
    switch (composition) {
        case 'one-four': // 四排
            // 监管elo分数不变
            break;
        case 'one-trio-one-solo': // 一组三排和一组单排
            adjustedHunterX -= 0.618;
            break;
        case 'four-solo': // 四个单排
            adjustedHunterX -= 2.472;
            break;
        case 'two-duo': // 两组双排
            adjustedHunterX -= 0.824;
            break;
        case 'one-duo-two-solo': // 一组双排两组单排
            adjustedHunterX -= 1.545;
            break;
    }
    
    // 计算新的差值
    const newX = adjustedHunterX - survivorTotalElo;
